...
def cos_similar(vec_a,vec_b):
        ...
        if normA == 0.0 or normB==0.0:
            ...
            return 1 - dot_product / ((normA*normB)**0.5)
...
class OnePassCluster:  
    ...
    def clustering(self):  
        ...
        for index in range(len(self.vectors))[1:]:
            ...
            '''
            min_distance = cos_similar(vec_a=self.vectors[0],vec_b=self.cluster_list[0].centroid)  
            ...
            for cluster_index, cluster in enumerate(self.cluster_list[1:]):  
                ...
                '''
                distance = cos_similar(vec_a=self.vectors[index],
                                              vec_b=cluster.centroid)
...
# 构建一趟聚类器  
clustering = OnePassCluster(vector_list=temperature_all_city, t=0.05)
